...
export default function FilterTags({
  ...
  return <div className="container mx-auto px-2 sm:px-4 py-2 sm:py-4">
      <div className="flex flex-wrap justify-center gap-1 sm:gap-2 mb-4">
        {availableTags.map(tag => {
        const isActive = activeTags.includes(tag.id);
        return <button key={tag.id} onClick={() => handleTagClick(tag.id)} className={cn("inline-flex items-center gap-1 sm:gap-1.5 px-1.5 sm:px-3 py-0.5 sm:py-1.5 rounded-full text-xs sm:text-sm font-medium transition-all duration-200", getTagColor(tag.type, isActive), "hover:scale-105 active:scale-95")}>
              {tag.icon}
              <span>{tag.label}</span>
              {isActive && tag.id !== 'all' && <X className="w-2 h-2 sm:w-3 sm:h-3 ml-0.5 sm:ml-1" />}
            </button>;
      })}
      </div>
